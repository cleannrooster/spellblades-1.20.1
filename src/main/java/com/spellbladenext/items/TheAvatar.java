...
public class TheAvatar extends TrinketItem {
    ...
    public Text getName(ItemStack stack) {
        ...
        double var2 = 0;
        double var3 = 0;
        if(stack.getOrCreateNbt().getString("1") != "") {
            if (stack.getOrCreateNbt().getString("1") == "arcane") {
                var01 = var01 + 1;
                 var1 +=  (stack.getOrCreateNbt().getInt("1modifier") * 0.01);

            }
            if (stack.getOrCreateNbt().getString("2") == "frost") {
                var02 = var02 + 1;
                 var2 +=  (stack.getOrCreateNbt().getInt("2modifier") * 0.01);

            }
            if (stack.getOrCreateNbt().getString("3") == "fire") {
                var03 = var03 + 1;
                 var3 +=  (stack.getOrCreateNbt().getInt("3modifier") * 0.01);

            }
            SpellSchool school = SpellSchools.ARCANE;
            if (var01 >= var02 && var01 >= var03) {
                school = SpellSchools.ARCANE;
            }
            if (var02 >= var03 && var02 >= var01) {
                school = SpellSchools.FROST;
            }
            if (var03 >= var02 && var03 >= var01) {
                school = SpellSchools.FIRE;


            }
            if(var03 == var02 && var02 == var01){
    ...
    public Multimap<EntityAttribute, EntityAttributeModifier> getModifiers(ItemStack stack, SlotReference slot, LivingEntity entity, UUID uuid) {
        ...
        if (stack.getOrCreateNbt().getString("1") != "") {
            ...
            if (SpellSchools.getSchool(school.id.toString()) != null) {
                ...
                double var11 = (stack.getOrCreateNbt().getInt("1modifier") * 0.01);
                double var21 = (stack.getOrCreateNbt().getInt("2modifier") * 0.01);
                double var31 = (stack.getOrCreateNbt().getInt("3modifier") * 0.01);
                modifiers.put(attribute, new EntityAttributeModifier(uuid, "spellblades:avatar", (var11 + var21 + var31) / 3F, EntityAttributeModifier.Operation.MULTIPLY_BASE));
            }
        }
